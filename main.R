...
for (i in 1:scenarios) {
  ...
  datasets <- replicate(
    ...
    expr = simulateData(
      N_index = pc$N[i],
      N_target = N_tar,
      prog_strength = prog_strength,
      inter_strength,
      overlap = pc$overlap[i],
      X_corr = rho,
      truth = FALSE),
    simplify = FALSE
  )
...
# with maximum-likelihood estimation and bootstrapping
gcomp.ml.wrapper <- function(datasets, resamples) {
  ...
  gcomp.ml <- function(data, indices) {
    ...
    outcome.model <- glm(y ~ (trt * x1) + (trt * x2),
                         data = index.dat, family = binomial)
  ...
  boot.object <- boot::boot(data = ipd.index,
                            statistic = gcomp.ml,
                            R = resamples)
  ...
  conf.ints <- quantile(boot.object$t,
                        probs = c(.025, .975),
                        type = 6)
...
for (i in 1:scenarios) {
  ...
  save(variances,
       file = paste0("Results/GcompML/variances_", file.id, ".RData"))
  ...
  save(variances,
       file = paste0("Results/MIM/variances_", file.id, ".RData"))
...
simulation.metrics <- as.data.frame(matrix(nrow = 2 * scenarios, ncol =
                                             11)) # store performance metrics here
colnames(simulation.metrics) <- c(
  "N",
  "Overlap",
  "Method",
  "Bias",
...
plot.results <- function(scenario) {
  ...
  scenario.ates <- subset(ate.table, N == pc$N[scenario] &
                            Overlap == pc$overlap[scenario]) # point estimates for a scenario
  ridge.plot <- ggplot(scenario.ates, aes(x = ATE, y = Method, fill = Method)) +
    geom_density_ridges(alpha = 0.65) +
    geom_vline(xintercept = true.log.OR[scenario],
               linetype = "dashed",
               color = "red") +
...
table.results <- function(scenario) {
  # scenario: index for the scenario
  scenario.metrics <- subset(simulation.metrics, N == pc$N[scenario] &
                               Overlap == pc$overlap[scenario]) # metrics for a scenario
  display.table <- cbind(
    Method = scenario.metrics$Method,
    Bias = paste0(format(
      round(scenario.metrics$Bias, digits = 3), nsmall = 3
    ), " (", format(
      round(scenario.metrics$Bias.MCSE, digits = 3), nsmall = 3
    ), ")"),
    ESE = paste0(format(
      round(scenario.metrics$ESE, digits = 3), nsmall = 3
    ), " (", format(
      round(scenario.metrics$ESE.MCSE, digits = 3), nsmall = 3
    ), ")"),
    MSE = paste0(format(
      round(scenario.metrics$MSE, digits = 3), nsmall = 3
    ), " (", format(
      round(scenario.metrics$MSE.MCSE, digits = 3), nsmall = 3
    ), ")"),
    Coverage = paste0(format(
      round(scenario.metrics$Cov, digits = 3), nsmall = 3
    ), " (", format(
      round(scenario.metrics$Cov.MCSE, digits = 3), nsmall = 3
    ), ")")
  )
  table.grob <- tableGrob(display.table, theme = ttheme_minimal(base_size =
                                                                  7))
...
ridge.plot.s1 <- plot.results(scenario = 1) + ggtitle(expression(paste(
  italic(N), "=500, limited overlap (", kappa, "=0.5)"
)))
ridge.plot.s2 <- plot.results(scenario = 2) + ggtitle(expression(paste(
  italic(N), "=1000, limited overlap (", kappa, "=0.5)"
)))
ridge.plot.s3 <- plot.results(scenario = 3) + ggtitle(expression(paste(
  italic(N), "=2000, limited overlap (", kappa, "=0.5)"
)))
ridge.plot.s4 <- plot.results(scenario = 4) + ggtitle(expression(paste(
  italic(N), "=500, full overlap (", kappa, "=1)"
)))
ridge.plot.s5 <- plot.results(scenario = 5) + ggtitle(expression(paste(
  italic(N), "=1000, full overlap (", kappa, "=1)"
)))
ridge.plot.s6 <- plot.results(scenario = 6) + ggtitle(expression(paste(
  italic(N), "=2000, full overlap (", kappa, "=1)"
)))
